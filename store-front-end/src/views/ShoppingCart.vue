    ...
    <div class="content" v-if="getShoppingCart.length>0">
      <ul>
        ...
        <li class="header">
          ...
          <div class="pro-num">amount</div>
          <div class="pro-total">sum</div>
          <div class="pro-action">process</div>
        </li>
        <!-- 购物车表头END -->
      ...
      <div class="cart-bar">
        <div class="cart-bar-left">
          <span>
            <router-link to="/goods">continue to shop</router-link>
          </span>
          <span class="sep">|</span>
          <span class="cart-total">
            Total
            <span class="cart-total-num">{{getNum}}</span> items,
            <span class="cart-total-num">{{getCheckNum}}</span> are selected.
          </span>
        </div>
        <div class="cart-bar-right">
          <span>
            <span class="total-price-title">Sum：</span>
            <span class="total-price">${{getTotalPrice}}</span>
          </span>
          <router-link :to="getCheckNum > 0 ? '/confirmOrder' : ''">
            <div :class="getCheckNum > 0 ? 'btn-primary' : 'btn-primary-disabled'">Pay</div>
          </router-link>
        </div>
      </div>
      <!-- 购物车底部导航条END -->
